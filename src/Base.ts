...
export class Base {
   ...
   */
  static async findById<T extends { _id: ObjectId | string }>(
    this: BaseClass<T>,
    id: string | mongodb.ObjectId,
    options: mongodb.FindOptions<T> = {}
  ): Promise<T | null> {
   ...
   * @param options
   */
  static aggregate<T extends mongodb.Document>(
    this: BaseClass<T>,
   ...
   * @param options
   */
  static findOne<T extends mongodb.Document>(
    this: BaseClass<T>,
    filter: mongodb.Filter<T>,
    options: mongodb.FindOptions<T> = {}
  ): Promise<T | null> {
   ...
   */
  static findMany<T extends mongodb.Document>(
    this: BaseClass<T>,
    filter: mongodb.Filter<T>,
    options: mongodb.FindOptions<T> = {}
  ): Promise<T[]> {
  ...
  static save<T extends { _id: mongodb.ObjectId | string }>(
    ...
    return this.basil.useCollection(target, async (collection) => {
      ...
      const result: mongodb.UpdateResult | mongodb.Document = await collection.replaceOne(
        ...
        target.fields.decode(payload.entity) as T,
        options
      );
   ...
   */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  static deleteMany<T extends { [key: string]: any }>(
    this: BaseClass<T>,
    filter: mongodb.Filter<T>,
    options: mongodb.DeleteOptions = {}
  ): Promise<mongodb.DeleteResult> {
